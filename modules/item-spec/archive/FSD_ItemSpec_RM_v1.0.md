...
4. [Halaman Detail – Item Spec RM](#4-halaman-detail--item-spec-rm)
   - 4.1 [Struktur Halaman Detail](#41-struktur-halaman-detail)
   - 4.2 [Toolbar Actions](#42-toolbar-actions)
   - 4.3 [Section Header](#43-section-header)
   - 4.4 [Tab 1: Material Description](#44-tab-1-material-description)
   - 4.5 [Tab 2: Organoleptic](#45-tab-2-organoleptic)
   - 4.6 [Tab 3: Nutrition & Physical](#46-tab-3-nutrition--physical)
   - 4.7 [Tab 4: Microbiological](#47-tab-4-microbiological)
   - 4.8 [Tab 5: Heavy Metals](#48-tab-5-heavy-metals)
   - 4.9 [Tab 6: Other Contaminant](#49-tab-6-other-contaminant)
   - 4.10 [Tab 7: Food Category](#410-tab-7-food-category)
   - 4.11 [Universal Input Modal](#411-universal-input-modal)
   - 4.12 [LOV – Oracle Reference](#412-lov--oracle-reference)
   - 4.13 [LOV – RM Evaluation Reference](#413-lov--rm-evaluation-reference)
...
```mermaid
flowchart TD
    Start(["Mulai"]) --> CreateDoc["PDV Klik 'Create Item Spec RM'\n(ItemSpecRMIndex.html)"]
    CreateDoc --> GenDocNo["Sistem Generate:\n- Doc No (Otomatis)\n- Doc Date (Hari ini)"]
    GenDocNo --> SelectItem["Pilih Item Code\n(LOV Oracle Inventory)"]
    SelectItem --> AutoFill["Auto-fill:\n- Item Spec No = 'S-[ITEM CODE]'\n- Spec Description\n- Data awal RM Evaluation"]
    AutoFill --> RefCheck{"Gunakan\nReferensi?"}
    RefCheck -->|"Ya"| SelectRef["Pilih Ref. Type:\nOracle / RM Evaluation"]
    SelectRef --> LoadRef["Data ter-copy otomatis:\n- Material Description\n- Allergens\n- Semua tabel parameter"]
    LoadRef --> FillData
    RefCheck -->|"Tidak"| FillData["Isi Data Manual:\n1. Material Description\n2. Allergen\n3. Tab Parameter Uji"]
    FillData --> SaveDraft["Save Draft"]
    SaveDraft --> Review["PDV / QA Review\n& Submit"]
    Review --> Submitted["Status: Submitted in QA / QS\n(Notifikasi ke Approver)"]
    Submitted --> ApproverAction{"Keputusan\nApprover"}
    ApproverAction -->|"Approve"| Approved["Status: APPROVED"]
    ApproverAction -->|"Reject"| Rejected(["Status: REJECTED\n(kembali ke PDV)"])
    Approved --> SubmitOracle["Submit ke Oracle ERP"]
    SubmitOracle --> OracleResult{"Integrasi\nOracle"}
    OracleResult -->|"Berhasil"| Registered(["Status: Registered\n(Selesai)"])
    OracleResult -->|"Gagal"| ErrorOracle["Status: Error in ORACLE\n(Perlu dicek ulang)"]
    ErrorOracle --> SubmitOracle

    style Start fill:#8CC63F,color:#fff,stroke:#009B4D
    style Registered fill:#388E3C,color:#fff,stroke:#1B5E20
    style Rejected fill:#D32F2F,color:#fff,stroke:#B71C1C
    style ErrorOracle fill:#F57C00,color:#fff,stroke:#E65100
    style CreateDoc fill:#FFF3CD,stroke:#FFC107
    style Approved fill:#D1ECF1,stroke:#17A2B8
    style SubmitOracle fill:#E2E3E5,stroke:#6C757D
```
...
### 4.1 Struktur Halaman Detail

```
[Page Toolbar]
    ┌────────────────────────────────────────────────────────────────┐
    │  RM Selection / Item Spec RM /  Detail                         │
    │                        [Print] [Save] [Submit] [Back]          │
    └────────────────────────────────────────────────────────────────┘

[Prototype Role Toggle]  ← Simulate QA/QS Role (untuk development/testing)

[Section Header Card]
    Doc Date | Doc No | Item Code [LOV] | Item Spec No.
    Version  | Spec Description | Spec Type | Ref. Type [LOV] | Reference Item Spec [LOV]

[Tab Navigation]
    ┌──────────────┬───────────────┬──────────────┬──────────────┬─────────────┬──────────────────┬──────────────┐
    │ Material     │ Organoleptic  │ Nutrition &  │ Microbiolo-  │ Heavy       │ Other            │ Food         │
    │ Description  │               │ Physical     │ gical        │ Metals      │ Contaminant      │ Category     │
    └──────────────┴───────────────┴──────────────┴──────────────┴─────────────┴──────────────────┴──────────────┘

[Action Bar Tab]
    Parameter Data          [Add / Edit Row]
...
### 4.4 Tab 1: Material Description
...
#### 4.4.1 Material Properties Fields
...
#### 4.5.1 Kolom DataTable – Organoleptic
...
**Kolom DataTable** identik dengan Tab Organoleptic (lihat Bagian 4.5.1).
...
**Kolom DataTable** identik dengan Tab Organoleptic (lihat Bagian 4.5.1), kecuali tombol Action hanya muncul untuk QA/QS.
...
**Kolom DataTable** untuk masing-masing sub-tabel identik dengan Tab Organoleptic (lihat Bagian 4.5.1).
...
#### 4.10.1 Kolom DataTable – Food Category
...
**Catatan**: Tab Food Category menggunakan form berbeda di Universal Input Modal (lihat Bagian 4.11.2 – Food Category Fields).
...
#### 4.11.1 Common Form Fields (Tab 2–5 + Sub-tabel Other Contaminant)
...
#### 4.11.2 Food Category Fields (Tab Food Category)
...
### 4.13 LOV – RM Evaluation Reference
...
| Halaman Daftar            | Tabel biasa, tanpa ringkasan statistik          | Dashboard dengan 5 kartu summary interaktif             |
| Filter Status             | Tidak ada filter cepat                          | Klik kartu → filter tabel otomatis                      |
