...
| Step | Nama            | Deskripsi                                                 |
|------|-----------------|-----------------------------------------------------------|
| 1    | Document Sample | Pendaftaran dan dokumentasi sample RM dari supplier       |
| 2    | Sample Purpose  | Analisis tujuan dan kebutuhan sample, mapping ke produk   |
...
*(Gambar 1: Halaman Index – Dashboard dan Data Table)*
...
*(Gambar 2: Halaman Detail – 4-Step Wizard Form)*

---

#### 3.2.1 Step 1: Document Sample

*(Gambar 3: Step 1 – Document Sample)*

**Fungsi Utama:**
- Mendokumentasikan informasi dasar sample RM dari supplier
- Input data supplier, material, pricing, dan quantity
...
| Material Name  | `txtMaterialName` | Yes       | Min 3, Max 200 char       | Nama material yang di-sample                        |
| Supplier       | `txtSupplierName` | Yes       | LOV Selection             | Supplier yang mengajukan sample                     |
| Price          | `decPrice`        | No        | Numeric, >= 0             | Harga sample per UOM                                |
| Price Currency | `txtCurrencyId`   | Conditional| LOV Selection            | Wajib jika Price diisi (IDR / USD / EUR)            |
| UOM            | `txtUOM`          | Yes       | LOV Selection             | Unit of Measure (kg / liter / pcs)                  |
| Quantity       | `decQuantity`     | Yes       | Numeric, > 0              | Jumlah sample yang diterima                         |
| PIC            | `txtPIC`          | Yes       | Auto from Login User      | Person In Charge (default: user login)              |
| Submission Date| `dtSubmissionDate`| Auto      | Auto: Current Date        | Tanggal pengajuan sample                            |
| Remarks        | `txtRemarks`      | No        | Max 500 char              | Catatan tambahan                                    |
...
#### 3.2.2 Step 2: Sample Purpose

*(Gambar 4: Step 2 – Sample Purpose)*
...
| Field Name        | ID Elemen             | Mandatory | Validasi               | Keterangan                                            |
|-------------------|-----------------------|-----------|------------------------|-------------------------------------------------------|
| Purpose Category  | `txtPurposeCategory`  | Yes       | LOV Selection          | Tujuan (Cost Reduction / Quality Improvement / New Product / dll) |
| Target Product    | `txtTargetProduct`    | Yes       | LOV Multi-select       | Product yang akan menggunakan material ini            |
| Expected Benefit  | `txtExpectedBenefit`  | Yes       | Min 10, Max 1000 char  | Benefit yang diharapkan dari sample ini               |
| Usage Percentage  | `decUsagePercentage`  | No        | Numeric, 0–100         | Persentase usage dalam formula (%)                    |
| Replacement For   | `txtReplacementFor`   | No        | LOV Selection          | Material yang akan digantikan (jika ada)              |
...
| Field Name              | ID Elemen               | Mandatory | Validasi              | Keterangan                                         |
|-------------------------|-------------------------|-----------|------------------------|-----------------------------------------------------|
| Test Date               | `dtTestDate`            | Yes       | Date                  | Tanggal pelaksanaan testing                         |
| Test Type               | `txtTestType`           | Yes       | LOV Selection         | Jenis testing (Lab Test / Trial Production / Sensory Test) |
| Test Result             | `txtTestResult`         | Yes       | Selection: Pass / Fail| Hasil testing                                       |
| Specification Compliance| `txtSpecCompliance`     | Yes       | Selection: Yes/No/Partial | Kesesuaian dengan spesifikasi                   |
| Quality Score           | `intQualityScore`       | No        | Numeric, 0–100        | Skor kualitas material                              |
| Evaluation Notes        | `txtEvaluationNotes`    | Yes       | Min 10, Max 2000 char | Catatan detail hasil evaluasi                       |
| Evaluator               | `txtEvaluator`          | Yes       | LOV Selection         | Nama evaluator/tester                               |
| Potensi EG/DEG          | `txtPotensiEGDEG`       | No        | Multi-select LOV      | Potensi bahan mengandung Ethylene/Diethylene Glycol |

**Test Result Attachment:**
- Upload hasil test report, foto sample, dll.
- Multiple files diperbolehkan per entry evaluasi.

---

#### 3.2.4 Step 4: Disposition

*(Gambar 6: Step 4 – Disposition)*
...
*(Gambar 7: ERD – Relasi Tabel Modul New RM Sample)*

### 4.3 Tabel Transaksi – 11 Tabel

Total tabel transaksi yang dibuat untuk modul ini: **11 tabel**, terbagi per step workflow.

#### STEP 1 – Document Sample (5 Tabel)

**4.3.1 `trRMSample_Header`** — Unified Header Seluruh Workflow

Satu record per RM Sample. Menjadi parent untuk keempat step.

| Kolom | Tipe Data | Nullable | Default | Keterangan |
|-------|-----------|----------|---------|------------|
| `intId` | INT IDENTITY (PK) | No | Auto | Primary key, FK reference |
| `txtId` | VARCHAR(50) (UK) | No | GUID | Business unique key |
| `txtSampleNo` | VARCHAR(50) (UK) | No | Auto | Format: SMPL-YYYY-XXXX |
| `intStatusId` | INT | No | 0 | 0=Draft, 1=InProgress, 2=WaitingApproval, 3=Approved, 4=Rejected, 5=OnHold |
| `intCurrentStep` | INT | No | 1 | Step aktif saat ini (1–4) |
...
**4.3.2 `trDocumentSample_Header`** — Step 1: Data Dokumentasi Sample

Menyimpan informasi material, supplier, halal, GMO, shelf life, packaging.
...
| `txtProductionMethod` | VARCHAR(200) | Yes | Metode produksi |
...
| `txtShippingMethod` | VARCHAR(200) | Yes | Metode pengiriman |
...
| `txtCurrencyCode` | VARCHAR(10) | No | Kode mata uang (IDR, USD) |
| `txtUOMCode` | VARCHAR(10) | No | Kode UOM (KG, LTR) |
...
| `bitContainPHO` | BIT | No | 0 | Mengandung PHO (0=No, 1=Yes) |

---

**4.3.3 `trDocumentSample_Allergen`** — Allergen Checklist
...
**4.3.4 `trDocumentSample_BTP`** — Carry Over BTP (Bahan Tambahan Pangan)
...
**4.3.5 `trDocumentSample_Document`** — Upload Dokumen Pendukung

Multiple rows, satu row per file yang diupload (COA, Spec, Halal Cert, dll).
...
| `intSampleHeaderId` | INT (FK) | FK → `trRMSample_Header.intId` |
| `intDocSampleHeaderId` | INT (FK) | FK → `trDocumentSample_Header.intId` |
| `txtDocumentType` | VARCHAR(50) | Specification / COA / Halal Certificate / Price List / Regulation / Flow Process |
...
| `txtSpecNumber` | VARCHAR(100) | Nomor spesifikasi (opsional) |
| `txtCertificateNo` | VARCHAR(100) | Nomor sertifikat (opsional) |
| `dtExpiredDate` | DATE | Tanggal expired (opsional) |
| `txtRemarks` | VARCHAR(MAX) | Keterangan |

---

#### STEP 2 – Sample Purpose (2 Tabel)

**4.3.6 `trSamplePurpose_Header`** — Data Purpose Utama
...
**4.3.7 `trSamplePurpose_Product`** — Apply To Product (Child Table)
...
**4.3.8 `trRMEvaluation_Header`** — Summary Evaluasi
...
**4.3.9 `trRMEvaluation_Detail`** — Parameter Uji (Testing Parameters)
...
**4.3.10 `trRMEvaluation_FoodCategory`** — Food Category Mapping
...
**4.3.11 `trRMDisposition_Header`** — Keputusan Final & Approval
...
### 4.4 Tabel Master yang Digunakan (Existing — Tidak Perlu Dibuat Ulang)

> Tabel berikut **sudah ada** di database `IDC_Formulation` / `KN2017_Formulation`. Cukup dijadikan referensi FK saja.

| Nama Tabel | DB Asal | Digunakan di | Keterangan |
|------------|---------|--------------|------------|
| `mDepartment` | KN2017_Formulation | LOV Departemen | Departemen user |
| `mPackingType` | KN2017_Formulation | Step 1 | Jenis kemasan |
| `mObjective` | KN2017_Formulation | Step 2 | Tujuan penggunaan |
| `mOrganization` | KN2017_Formulation | LOV Organisasi | Divisi/Organisasi |
| `mSampleType` | KN2017_Formulation | Step 1 | Jenis sample |
| `mAppParam` | IDC_Formulation | Step 1 | Storage condition, app parameter |
...
4. **Catatan Reject** (Reason) wajib minimal 10 karakter untuk semua jenis keputusan disposisi.
5. **Analysis Deadline** tidak boleh kurang dari tanggal hari ini.
6. **Quality Score** harus dalam rentang 0–100 jika diisi.

### 5.4 Upload Dokumen

1. Format file yang diizinkan: **PDF, DOC, DOCX, XLS, XLSX, JPG, PNG**. Ukuran maksimum: **10 MB** per file.
2. Multiple file diperbolehkan per sample dan per step evaluasi.
...
4. Hanya PIC atau user dengan role Admin yang dapat membatalkan (*cancel*) sample saat masih status **Draft**.

### 5.6 Proses Approval

1. Approver menerima notifikasi saat status menjadi `PENDING_APPROVAL`.
2. Jika Approver **Approve** → status berubah menjadi `COMPLETED`.
3. Jika Approver **Reject** → status kembali ke Step 4 untuk revisi keputusan.
4. Approval note wajib diisi saat Approver melakukan Reject.
5. Approver yang sudah memberikan keputusan tidak dapat mengubahnya kembali.

### 5.7 On Hold

1. Jika keputusan disposisi adalah **On Hold**, user wajib mengisi Reason dan Next Action.
2. Sample dengan status **On Hold** dapat kembali ke Step 3 (RM Evaluation) untuk evaluasi ulang.
3. Timeline evaluasi ulang ditentukan oleh Analysis Deadline yang baru (opsional).
...
| UOM                   | `M_UOM`          | `SELECT * FROM M_UOM WHERE bitActive = 1`                                          | `txtUOM`                         |
| Purpose Category      | `M_PURPOSE_CATEGORY`| `SELECT * FROM M_PURPOSE_CATEGORY WHERE bitActive = 1`                          | `txtPurposeCategory`             |
| Target Product        | Oracle EBS / IDC | Product list dari sistem Oracle atau IDC                                           | `txtTargetProduct`               |
| Replacement Material  | `M_MATERIAL`     | `SELECT * FROM M_MATERIAL WHERE bitActive = 1`                                     | `txtReplacementFor`              |
| Test Type             | `M_TEST_TYPE`    | `SELECT * FROM M_TEST_TYPE WHERE bitActive = 1`                                    | `txtTestType`                    |
| Potensi EG/DEG        | Hardcoded        | Daftar bahan mengandung Ethylene Glycol / Diethylene Glycol                        | `txtPotensiEGDEG` (multi-select) |
| Evaluator / PIC       | `M_PEGAWAI`      | `SELECT * FROM M_PEGAWAI WHERE bitActive = 1`                                      | `txtEvaluator`, `txtPIC`         |
| Approver              | `M_PEGAWAI`      | `SELECT * FROM M_PEGAWAI WHERE bitActive = 1 AND txtRole IN ('MANAGER','ADMIN')`   | `txtApproverId`                  |

> **Catatan**: Field `txtPotensiEGDEG` menggunakan komponen **Select2 multi-select** agar user bisa memilih lebih dari satu bahan sekaligus. Data bersifat hardcoded dalam sistem (tidak dari database).
...
**Catatan:**
- Kolom **Edit Sample (Step 3 only)**: Lab/QC Team hanya bisa mengisi data di Step 3 (RM Evaluation), field lain read-only.
- Kolom **Approve**: Manager/Administrator hanya dapat melakukan approval jika terdaftar sebagai Approver pada disposisi tersebut.
- Semua user dapat melihat (*View*) sample yang dibuat oleh user lain dalam divisi yang sama.
...
| Sample masuk ke Step 2 (Purpose)   | R&D Team terkait                  | In-App          |
...
> **Referensi lengkap:** lihat folder `idc-system/Database/Scripts/`
...
### A.2 `trDocumentSample_Header` — Step 1: Document Sample
